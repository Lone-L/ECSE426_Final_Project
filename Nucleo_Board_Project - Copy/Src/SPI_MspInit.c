...
void HAL_SPI_MspInit(SPI_HandleTypeDef* hspi)
{
  ...
  if(hspi->Instance==BNRG_SPI_INSTANCE)
    ...
    /* Enable GPIO Ports Clock */ 
		...
		DISCOVERY_SPI_CS_CLK_ENABLE();
		DISCOVERY_SPI_ACCEL_DATAREADY_CLK_ENABLE();
		DISCOVERY_SPI_TEMP_DATAREADY_CLK_ENABLE();
		...
		GPIO_InitStruct.Mode  = DISCOVERY_SPI_CS_MODE;
		GPIO_InitStruct.Pull  = DISCOVERY_SPI_CS_PULL;
		GPIO_InitStruct.Speed = DISCOVERY_SPI_CS_SPEED;
		GPIO_InitStruct.Alternate = DISCOVERY_SPI_CS_ALTERNATE;
		HAL_GPIO_Init(DISCOVERY_SPI_CS_GPIO_PORT, &GPIO_InitStruct);
		HAL_GPIO_WritePin(DISCOVERY_SPI_CS_GPIO_PORT, DISCOVERY_SPI_CS_PIN, GPIO_PIN_SET);
		
		/* Configure GPIO PINs to detect Interrupts */
		/* ACCEL_DATAREADY pin configuration */
		GPIO_InitStruct.Pin   = DISCOVERY_SPI_ACCEL_DATAREADY_PIN;
		GPIO_InitStruct.Mode  = DISCOVERY_SPI_ACCEL_DATAREADY_MODE;
		GPIO_InitStruct.Pull	= DISCOVERY_SPI_ACCEL_DATAREADY_PULL;
		GPIO_InitStruct.Speed = DISCOVERY_SPI_ACCEL_DATAREADY_SPEED;
		GPIO_InitStruct.Alternate = DISCOVERY_SPI_ACCEL_DATAREADY_ALTERNATE;
		HAL_GPIO_Init(DISCOVERY_SPI_ACCEL_DATAREADY_PORT, &GPIO_InitStruct);

		/* TEMP_DATAREADY pin configuration */
		GPIO_InitStruct.Pin   = DISCOVERY_SPI_TEMP_DATAREADY_PIN;
		GPIO_InitStruct.Mode  = DISCOVERY_SPI_TEMP_DATAREADY_MODE;
		GPIO_InitStruct.Pull	= DISCOVERY_SPI_TEMP_DATAREADY_PULL;
		GPIO_InitStruct.Speed = DISCOVERY_SPI_TEMP_DATAREADY_SPEED;
		GPIO_InitStruct.Alternate = DISCOVERY_SPI_TEMP_DATAREADY_ALTERNATE;
		HAL_GPIO_Init(DISCOVERY_SPI_TEMP_DATAREADY_PORT, &GPIO_InitStruct);
		...
		HAL_NVIC_SetPriority(DISCOVERY_SPI_ACCEL_IRQn, 0, 0);
		HAL_NVIC_EnableIRQ(DISCOVERY_SPI_ACCEL_IRQn);
		
		HAL_NVIC_SetPriority(DISCOVERY_SPI_TEMP_IRQn, 0, 0);
